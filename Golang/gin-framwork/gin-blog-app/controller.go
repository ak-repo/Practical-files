...
func RegistrationHandler(c *gin.Context) {
	...
	//check already exist
	if _, ok := UsersDB[newUser.Username]; ok {
		c.JSON(http.StatusConflict, gin.H{"error": "username already taken"})
		return
	}

	//password hashing
	hashedPassword, _ := GenerateHashedPassword(newUser.Password)
...
func LoginHandler(c *gin.Context) {
	...
	var loginUser UserLoginInput
	if err := c.ShouldBindJSON(&loginUser); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	user, ok := UsersDB[loginUser.Username]

	//verification:= password, username
	if !ok || CompareHashAndPassword(loginUser.Password, user.HashedPassword) {
